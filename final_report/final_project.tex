...
\teaser{
	...
	$\begin{array}{@{\hspace{-0.00in}}c@{\hspace{0.10in}}c@{\hspace{0.10in}}c@{\hspace{0.05in}}c}
        ...
        &\includegraphics[width=2.2in]{Images/ResNet-PCA-3d-mesh.png}
			...
			(a)  &  (b) & (c)
		\end{array}$
	\caption{The loss landscapes the ResNet-50 model showing the gradient decent path using PCA dimensionality reduction in (a) 2D with contours, (b) 3D with contours and mesh base, and (c) 3D with only mesh } \label{fig:teaser} }
...
We constructed our CNNs in Python using the Pytorch library and then train and test our models on the CIFAR-10 dataset (\url{https://www.cs.toronto.edu/~kriz/cifar.html}). The CIFAR-10 dataset is a collection of 6000 color images that are 32x32 pixels and contain 10 separate classes (see Figure 2 for examples). The models were then trained using a batch size of 10, run for 10 training epochs, and evaluated using cross entropy as the loss function (\url{https://pytorch.org/docs/stable/generated/torch.nn.CrossEntropyLoss.html}).
